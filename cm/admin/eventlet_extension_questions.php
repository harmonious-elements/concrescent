...
function render_extension_questions($connection) {
	$results = mysql_query('SELECT * FROM '.db_table_name('eventlet_extension_questions').' ORDER BY `order`', $connection);
	while ($result = mysql_fetch_assoc($results)) {
		$result = decode_extension_question('eventlet', $result);
...
if (isset($_POST['action'])) {
	$id = (int)$_POST['id'];
	switch ($_POST['action']) {
		case 'activate': activate_entity('eventlet_extension_questions', $id, $conn); break;
		case 'deactivate': deactivate_entity('eventlet_extension_questions', $id, $conn); break;
		case 'delete': delete_entity('eventlet_extension_questions', $id, $conn); break;
		case 'reorder': reorder_entities('eventlet_extension_questions', $id, (int)$_POST['direction'], $conn); break;
		case 'save': upsert_ordered_entity('eventlet_extension_questions', $id, encode_extension_question('eventlet', $_POST), $conn); break;
	}
	render_extension_questions($conn);
	exit(0);
}

render_admin_head('Panel/Activity Extra Questions');
...
listPage({
	ajaxUrl: 'eventlet_extension_questions.php',
...
render_admin_body('Panel/Activity Extra Questions');

...
require_once dirname(__FILE__).'/../lib/ui/staffers.php';

$conn = get_db_connection();
db_require_table('staffer_badges', $conn);
db_require_table('staffers', $conn);

function render_staffer_badges($connection) {
	...
	while ($result = mysql_fetch_assoc($results)) {
		...
		$purchased = get_purchased_staffer_badge_count($result['id'], $connection);
...
if (isset($_POST['action'])) {
	$id = (int)$_POST['id'];
	switch ($_POST['action']) {
		case 'activate': activate_entity('staffer_badges', $id, $conn); break;
		case 'deactivate': deactivate_entity('staffer_badges', $id, $conn); break;
		case 'delete': delete_entity('staffer_badges', $id, $conn); break;
		case 'reorder': reorder_entities('staffer_badges', $id, (int)$_POST['direction'], $conn); break;
		case 'save': upsert_ordered_entity('staffer_badges', $id, encode_staffer_badge($_POST), $conn); break;
	}
	render_staffer_badges($conn);
	exit(0);
}

render_admin_head('Staff Badges');
...
render_admin_body('Staff Badges');
...
), 'render_staffer_badges', true, $conn);
echo '</div>';

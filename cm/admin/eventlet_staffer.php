...
if (isset($_POST['action'])) {
	...
	switch ($_POST['action']) {
		case 'save':
			$set = encode_eventlet_staffer($_POST);
			if ($id) {
				$q = 'UPDATE '.db_table_name('eventlet_staffers').' SET '.$set.' WHERE `id` = '.$id;
				mysql_query($q, $conn);
			} else {
...
if ($id) {
	$results = mysql_query('SELECT * FROM '.db_table_name('eventlet_staffers').' WHERE `id` = '.$id, $conn);
	$result = mysql_fetch_assoc($results);
	$result = decode_eventlet_staffer($result, $eventlet_info);
	$name = $result['real_name'];
	$eid = $result['eventlet_id'];
} else {
	$result = null;
	$name = null;
	$eid = (isset($_GET['eventlet_id']) && $_GET['eventlet_id']) ? (int)$_GET['eventlet_id'] : null;
}

render_admin_head($name ? ('Edit Panelists/Hosts - '.$name) : 'Add Panelists/Hosts');
...
render_admin_body($name ? 'Edit Panelists/Hosts' : 'Add Panelists/Hosts');

echo '<div class="card">';
	echo '<form action="eventlet_staffer.php?id='.$id.'" method="post">';
		echo '<div class="card-content">';
			if ($changed) {
				echo '<div class="notification">Changes saved.</div>';
			}
			echo '<table border="0" cellpadding="0" cellspacing="0" class="form entity-record eventlet-staffer-record">';
				echo '<thead><tr><th colspan="2"><div class="subhead" id="sh-per">Personal Information</div></th></tr></thead>';
				echo '<tbody class="sh-per">';
					echo '<tr>';
						echo '<th><label for="eventlet_id">Panel/Activity:</label></th>';
						echo '<td><select name="eventlet_id">';
							foreach ($eventlet_info as $eventlet_id => $eventlet) {
								echo '<option value="'.$eventlet_id.'"';
								if ($eid && $eid == $eventlet_id) echo ' selected="selected"';

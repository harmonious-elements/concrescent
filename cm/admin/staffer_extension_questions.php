...
function render_extension_questions($connection) {
	$results = mysql_query('SELECT * FROM '.db_table_name('staffer_extension_questions').' ORDER BY `order`', $connection);
	while ($result = mysql_fetch_assoc($results)) {
		$result = decode_extension_question('staffer', $result);
...
if (isset($_POST['action'])) {
	$id = (int)$_POST['id'];
	switch ($_POST['action']) {
		case 'activate': activate_entity('staffer_extension_questions', $id, $conn); break;
		case 'deactivate': deactivate_entity('staffer_extension_questions', $id, $conn); break;
		case 'delete': delete_entity('staffer_extension_questions', $id, $conn); break;
		case 'reorder': reorder_entities('staffer_extension_questions', $id, (int)$_POST['direction'], $conn); break;
		case 'save': upsert_ordered_entity('staffer_extension_questions', $id, encode_extension_question('staffer', $_POST), $conn); break;
	}
	render_extension_questions($conn);
	exit(0);
}

render_admin_head('Staff Extra Questions');
...
listPage({
	ajaxUrl: 'staffer_extension_questions.php',
...
render_admin_body('Staff Extra Questions');

...
if (isset($_POST['start_id'])) {
	...
	while ($result = mysql_fetch_assoc($results)) {
		...
		$html_content = render_list_row(
			...
			/* reorderable = */ false,
			/*        edit = */ false,
			/*      review = */ ('review_eventlet.php?id='.$result['id'])
		);
		$eventlets[] = array(
			'id' => $result['id'],
...
render_admin_head('Review Panel/Activity Applications');
...
?><script type="text/javascript">listPage({
	ajaxUrl: 'review_eventlets.php',
	entityType: 'panel/activity applications',
	progressive: true,
	searchable: true,
});</script><?php

render_admin_body('Review Panel/Activity Applications');

...
if (isset($_POST['start_id'])) {
	...
	while ($result = mysql_fetch_assoc($results)) {
		...
		$html_content = render_list_row(
			...
			/* reorderable = */ false,
			/*        edit = */ false,
			/*      review = */ ('review_staffer.php?id='.$result['id'])
		);
		$staffers[] = array(
			'id' => $result['id'],
...
render_admin_head('Review Staff Applications');
...
?><script type="text/javascript">listPage({
	ajaxUrl: 'review_staffers.php',
	entityType: 'staff applications',
	progressive: true,
	searchable: true,
});</script><?php

render_admin_body('Review Staff Applications');

$(document).ready(function() {
	var onBadgeIdChange = function() {
		...
		rate_num_tables = '';
		rate_num_staffers = '';
		if (badge['price_per_table']) {
			rate_num_tables += ' @ ' + badge['price_per_table_string'] + ' each';
		}
		if (badge['price_per_staffer']) {
			rate_num_staffers += ' @ ' + badge['price_per_staffer_string'] + ' each';
			if (badge['staffers_in_table_price']) {
				rate_num_staffers += ' (' + badge['staffers_in_table_price'] + ' included per table)';
			}
		}
		if (badge['max_tables']) {
			rate_num_tables += ' (maximum ' + badge['max_tables'] + ')';
			$('.epa-num-tables').attr('max', badge['max_tables']);
		} else {
			$('.epa-num-tables').removeAttr('max');
		}
		...
		$('.rate-num-tables').text(rate_num_tables);
